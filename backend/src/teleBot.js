...
const useWebhook = !!process.env.TG_WEBHOOK_URL; 

export const bot = useWebhook
  ? new TelegramBot(TELEGRAM_TOKEN, { webHook: true })
  : new TelegramBot(TELEGRAM_TOKEN, { polling: { interval: 500, autoStart: false } });

  // start polling hanya sekali
if (!useWebhook && !globalThis.__BOT_POLLING_STARTED__) {
  bot.startPolling();
  globalThis.__BOT_POLLING_STARTED__ = true;
}

// webhook mode
if (useWebhook) {
  const url = `${process.env.TG_WEBHOOK_URL}/bot${TELEGRAM_TOKEN}`;
  bot.setWebHook(url);
}
