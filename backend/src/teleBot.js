...
bot.onText(/^([\d.]+)\s+([\d.]+)$/, async (msg, match) => {
  ...
  if (isNaN(minV) || isNaN(maxV)) {
    return reply(chatId,
      '⚠️ Format tidak valid. Masukkan dua angka (min max), contoh: `24 27`',
      { parse_mode: 'Markdown' }
    );
  }
...
async function handleSensor(prefix, param, message, state) {
  ...
  if (step === 3 && prefix === 'A') {
    ...
    pendingStore.set(ud.id, { deviceId: ud.id, deviceName: ud.deviceName, userId, enumType: state.typeKey, minValue: setting.minValue, maxValue: setting.maxValue, enabled });
